...
execute as @a[tag=battou,scores={use=1}] at @s run summon armor_stand ~ ~ ~ {NoAI:1b,Invulnerable:1b,NoGravity:1b,Marker:true,Tags:["battoukyori"]}
...
execute as @a[tag=battou,scores={effecttime=0,actiontime=0..10}] at @s run function test:skill/battou/battouscan

execute as @a[tag=battou,scores={effecttime=0}] at @s if entity @e[tag=battoukyori,distance=..1] run effect clear @s

execute as @a[tag=battou,scores={actiontime=0..10,effecttime=0}] at @s if entity @e[tag=battoukyori,limit=1,sort=nearest] run particle flash ~ ~ ~ 0.1 0.1 0.1 0.1 10 normal @a

execute as @e[tag=battou,scores={actiontime=0..10,effecttime=0}] at @s run function test:skill/battou/batdamage

execute as @a[tag=battou,scores={effecttime=0,actiontime=0..10}] at @s run kill @e[tag=battoukyori,limit=1,sort=nearest] 

scoreboard players set @a[tag=battou,scores={actiontime=0..10,effecttime=0}] actiontime -1

scoreboard players set @a[scores={use=1},tag=battou] use 0
